...
def main():
    ...
    vuelo      = pd.read_csv("~/Downloads/vuelo.csv")    
    aeropuerto = pd.read_csv("~/Downloads/aeropuerto.csv")    
    pasajero   = pd.read_csv("~/Downloads/pasajero.csv")    
    reserva    = pd.read_csv("~/Downloads/reserva.csv")    
    # Ejercicio JOIN tuplas espúreas
    empleadoRol= pd.read_csv("~/Downloads/empleadoRol.csv")    
    rolProyecto= pd.read_csv("~/Downloads/rolProyecto.csv")    
    # Ejercicios funciones de agregación, LIKE, Elección, Subqueries y variables de Python
    examen     = pd.read_csv("~/Downloads/examen.csv")
    # Ejercicios de manejo de valores NULL
    examen03 = pd.read_csv("~/Downloads/examen03.csv")
    ...
    consultaSQL = """SELECT count(*) AS cantidadExamenes
                     FROM examen;
                  """
    ...
    consultaSQL = """SELECT Instancia, AVG(Edad) AS PromedioEdad
                     FROM examen
                     GROUP BY Instancia
                     ORDER BY Instancia ASC;
                  """
    ...
    consultaSQL = """SELECT Nombre, instancia, Nota
                     FROM examen
    ...
    consultaSQL = """
                     SELECT DISTINCT est.*, exa.Nota AS Parcial_02
                     FROM datosHastaParcial01 est
                     LEFT JOIN examen exa
    ...
    consultaSQL = """
                     SELECT DISTINCT est.*, exa.Nota AS Recuperatorio_01
    ...
    consultaSQL = """
                     SELECT DISTINCT est.*, exa.Nota AS Recuperatorio_02
                     FROM datosHastaRecuperatorio01 est
                     LEFT JOIN examen exa
    ...
    consultaSQL = """
                     SELECT DISTINCT est.*, 
                            CASE WHEN (      ( (Parcial_01 >= 4) OR (Recuperatorio_01 >=4) )
                                 ...
                                 ELSE 'NO APROBÓ'
                            END AS Estado
                     FROM datosHastaRecuperatorio02 est
                  """
